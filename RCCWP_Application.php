...
class RCCWP_Application
{
	...
	*/
	...
	function CheckInstallation(){
		...
		if ($dir_list != ""){
			echo "<div id='magic-fields-install-error-message-2' class='error'><p><strong>".__('Magic Fields is not ready yet.', $mf_domain)."</strong> ".__('The following folders must be writable (usually chmod 777 is necessary):', $mf_domain)."</p><ul>";
			echo $dir_list;
			echo "</ul></div>";
		} else {
                    /* the directory is writable
                     * we can create the css an js files
                     */
        ...
        function create_EditnPlace_js($create=FALSE) {
            ...
            $editnplace_js_file = MF_UPLOAD_FILES_DIR.'editnplacepath.js';
