	...
	public static function DisplayGroupFields($customGroupId, $intended = false) {
        ...
        <td>
          ...
          <input type="hidden" name="mf_order[<?php print $customGroupId;?>][]" value="<?php echo $field->id; ?>" />
				...
				?></td>
		  	<td><a onclick="return confirmBeforeDelete();" href="<?php echo wp_nonce_url(RCCWP_ManagementPage::GetCustomWritePanelGenericUrl('delete-custom-field')."&custom-field-id=$field->id",'delete-custom-field'); ?>" >&times; <?php _e('Delete',$mf_domain); ?></a></td>
			</tr>
		<?php
		endforeach;
	}
